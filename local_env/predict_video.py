...
ret, frame = cap.read()
H, W, _ = frame.shape
out = cv2.VideoWriter(video_path_out, cv2.VideoWriter_fourcc(*'mpv4'), int(cap.get(cv2.CAP_PROP_FPS)), (W, H))
...
while ret:
    ...
    for result in results.boxes.data.tolist():
        ...
        if score > threshold:
            ...
            cv2.putText(frame, f'{results.names[int(class_id)]} {score:.2f}'.upper(), (int(x1), int(y1 - 10)),
